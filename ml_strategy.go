...
func (ml *MLModel) ExtractFeatures(ts *techan.TimeSeries, index int) *TradingFeatures {
	...
	// Volume features (simplified - would need actual volume data)
	features.VolumeRatio = 1.0 // Placeholder
	features.VolumeMA = 1.0    // Placeholder
	...
	features.BollingerWidth = (upperBB - lowerBB) / sma
	features.BollingerPosition = (currentPrice - lowerBB) / (upperBB - lowerBB)
	...
	openPrice := currentPrice // Simplified - would use actual open price
	features.CandleBody = math.Abs(currentPrice - openPrice) / currentPrice
	features.CandleWick = ((highPrice - math.Max(currentPrice, openPrice)) + 
		                   (math.Min(currentPrice, openPrice) - lowPrice)) / currentPrice
...
func (ml *MLModel) calculateFeatureStats() {
	...
	featureNames := []string{
		...
		"MACDHistogram", "ATR", "BollingerWidth", "BollingerPosition",
	}
...
func (ml *MLModel) trainLinearModel(trainData []TradingFeatures) error {
	...
	for epoch := 0; epoch < epochs; epoch++ {
		...
		for _, data := range trainData {
			prediction := ml.predictValue(data)
			error := data.FutureReturn - prediction
			totalError += error * error
			
			// Update weights (simplified gradient descent)
			ml.trainedWeights["PriceChange1"] += learningRate * error * data.PriceChange1
			ml.trainedWeights["PriceChange5"] += learningRate * error * data.PriceChange5
			ml.trainedWeights["PriceChange10"] += learningRate * error * data.PriceChange10
			ml.trainedWeights["RSI"] += learningRate * error * data.RSI
			ml.trainedWeights["EMASpread"] += learningRate * error * data.EMASpread
			ml.trainedWeights["MACDHistogram"] += learningRate * error * data.MACDHistogram
			ml.trainedWeights["ATR"] += learningRate * error * data.ATR
			ml.trainedWeights["BollingerPosition"] += learningRate * error * data.BollingerPosition
			ml.trainedWeights["bias"] += learningRate * error
		}
		
		ml.performance.TrainingLoss = totalError / float64(len(trainData))
